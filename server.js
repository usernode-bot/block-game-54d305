...
const BLOCK_POINTS = {
  ...
  17: 5,  // Rainbow Block
  18: 3,  // Crystal Block
  19: 5, 20: 5, 21: 5, // Exclusive (Neon, Prism, Void Core)
};
...
app.post('/api/block', async (req, res) => {
  try {
    ...
    res.json({ ok: true, seq, ...(challenge ? { challenge } : {}), earned, combo_multiplier, rainbow_multiplier, newly_earned_badges, newly_unlocked_types, lines_cleared, line_clear_points, ...(spendable_coins !== null ? { spendable_coins } : {}) });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
